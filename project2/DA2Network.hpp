...
public:
  ...
  DA2Network(const char *network_path)
  {
    ...
    std::wstring w_network_path = converter.from_bytes(network_path);

    // Use the converted std::wstring
    this->session_ = new Ort::Session(env, w_network_path.c_str(), Ort::SessionOptions{nullptr});
  }
